...
import { signupOtpKey, otpAttemptsKey, otpFreezeKey, otpResendKey } from './redisKeys';
...
export const storeOtp = async (sessionId: string, otpHash: string): Promise<void> => {
    // Stores: signup:otp:<sessionId> → { otp_hash }
    await redisClient.set(signupOtpKey(sessionId), JSON.stringify({ otp_hash: otpHash }), 'EX', OTP_TTL);
};

export const getStoredOtp = async (sessionId: string): Promise<string | null> => {
