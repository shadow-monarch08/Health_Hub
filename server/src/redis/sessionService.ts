...
export const createSignupSession = async (sessionId: string, sessionData: SignupSession): Promise<void> => {
    await redisClient.set(signupSessionKey(sessionId), JSON.stringify(sessionData), 'EX', SESSION_TTL);
};

export const getSignupSession = async (sessionId: string): Promise<SignupSession | null> => {
...
export const markSessionVerified = async (sessionId: string): Promise<void> => {
    ...
    if (session) {
        ...
        await redisClient.set(signupSessionKey(sessionId), JSON.stringify(session), 'EX', SESSION_TTL);
    }
};
