...
import { CleanRecord, NormalizedRecord } from "../common/ehrProvider.interface";

export class EpicCleaner {
    /**
     * Triggered after normalization. Aggregates data for a specific profile and resource type.
     */
    async clean(profileId: string, resourceType: string): Promise<CleanRecord | null> {
        try {
            ...
            await prisma.profileFhirResourceClean.upsert({
                ...
                update: {
                    cleanJson: cleanData,
                    sources: normalizedRecords.map((r: any) => ({
                        provider: r.provider,
                        raw_id: r.resourceId,
    ...
    private aggregateMedications(records: any[]): any {
        ...
        for (const record of records) {
            ...
            const medName =
                data.medication?.name || data.medication || "Unknown Medication";
    ...
    private aggregateConditions(records: any[]): any {
        ...
        for (const record of records) {
            ...
            if (!existing) {
                ...
                if (
                    data.onset &&
                    (!existing.onset_date || data.onset < existing.onset_date)
                ) {
...
export const epicCleaner = new EpicCleaner();
