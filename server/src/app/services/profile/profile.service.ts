  ...
  async getProfileData(profileId: string): Promise<any> {
    ...
    records.forEach((record) => {
      switch (record.resourceType) {
        case "Patient":
          data.patient = record.cleanJson; // single resource is OK
          break;

        case "Condition":
          data.conditions.push(record.cleanJson);
          break;

        case "AllergyIntolerance":
          data.allergies.push(record.cleanJson);
          break;

        case "MedicationRequest":
          data.medications.push(record.cleanJson);
          break;

        case "Observation":
          data.labs.push(record.cleanJson);
          break;

        case "Encounter":
          data.encounters.push(record.cleanJson);
          break;

        case "Procedure":
          data.procedures.push(record.cleanJson);
          break;

        case "Immunization":
          data.immunizations.push(record.cleanJson);
          break;
      }
    });
