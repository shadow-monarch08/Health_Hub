...
export class SyncService {
  ...
  async syncProfile(
    ...
    logger.info(
      `Starting Sync Job ${jobId} for profile ${profileId} (${providerName})`
    );
    ...
    try {
      ...
      redisClient.set(
        cooldownKey(profileId, providerName),
        "1",
        "PX",
        SYNC_COOLDOWN_MS
      );
      ...
      sseRedis.publish(
        `sse:${jobId}`,
        JSON.stringify({ event: "complete", resourceType: "all" })
      );
  ...
  async createSyncJob(
    ...
    const syncStatus = await syncStatusService.resolveSyncStatus(
      profileId,
      provider
    );
    const frontendRedirect =
      process.env.FRONTEND_REDIRECT ||
      "https://frontend.example.com/epic/success";
