...
export class OAuthService {
     ...
     */
    async createAuthorizationRedirect(userId: string, profileId: string): Promise<string> {
        const state = uuidv4();
        const codeVerifier = this.generateCodeVerifier();
        const codeChallenge = this.generateCodeChallenge(codeVerifier);

        // Store state with metadata
        await redisOAuth.saveState(state, { userId, profileId, provider: 'epic', codeVerifier });
     ...
     */
    async exchangeCodeForToken(state: string, code: string): Promise<{ token: TokenResponse, stateData: any }> {
     ...
     */
    async storeConnection(userId: string, profileId: string, tokenData: TokenResponse): Promise<void> {
