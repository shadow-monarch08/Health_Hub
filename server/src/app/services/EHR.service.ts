...
import { sendEvent } from "../sse/sseManager";
...
export class EHRService {
  ...
  async syncResource(
    ...
    // Emit SSE event for fetching
    sendEvent(jobId, "fetching", resourceType);
    ...
    if (!response.ok) {
      ...
      // Emit SSE event for fetching
      sendEvent(jobId, "failed", resourceType);
    ...
    // Emit SSE event for fetching
    sendEvent(jobId, "fetched", resourceType);
    ...
    // Emit SSE event for fetching
    sendEvent(jobId, "normalizing", resourceType);
